...
- `src/components/HostDiagram.jsx` — diagram kotak-garis: host di atas, anak-anaknya di bawah dengan garis penghubung (bus horizontal + drop vertikal), murni CSS (`::before`/`::after`), nggak ada library/koordinat yang dihitung JS. Garisnya di-animasi "mengalir" (dash bergerak host→anak lewat `background-position` + `@keyframes`), dihormati `prefers-reduced-motion`
